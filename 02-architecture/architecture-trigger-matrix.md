...
| **400G Ethernet scale-out** | Retimed route remains possible but power/interface cost is unresolved | Current end-to-end evidence is modeled only; 212.5 GBd margin fails above 12 dB B2B in the reviewed model | Stronger current technical direction if a replaceable short module is qualified | Stronger direction where package density and channel loss bind | CPO/NPO are technically favored hypotheses, not proven adoption winners; require measured 212.5 GBd link and final package data [PAP-011; STD-009] |
